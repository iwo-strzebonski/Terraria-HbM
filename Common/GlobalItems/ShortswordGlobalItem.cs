...
namespace TerrariaHbM.Common.GlobalItems
{
	...
	// See other GlobalItem classes in TerrariaHbM to see other ways that GlobalItem can be used.
	public class ShortswordGlobalItem : GlobalItem
	{
		...
		public override void SetDefaults(Item item)
		{
			...
			item.damage = 100; // Change damage to 100!
		}

		public override bool Shoot(Item item, Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
		{
			// Make it shoot grenades for no reason
			Projectile.NewProjectileDirect(source, player.Center, velocity * 10f, ProjectileID.Grenade, damage, knockback, player.whoAmI);
