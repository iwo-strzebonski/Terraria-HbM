...
using TerrariaHbM.Content.BossBars;
...
namespace TerrariaHbM.Content.Bosses.BillCipher
{
	// The main part of the boss, usually referred to as "body"
	[AutoloadBossHead] // This attribute looks for a texture called "ClassName_Head_Boss" and automatically registers it as the NPC boss head icon
	public class BillCipherBoss : ModNPC
	{
		...
		public override string Texture => TerrariaHbM.AssetPath + $"Textures/Bosses/BillCipher/{Name}";
		...
		{
			return ModContent.NPCType<BillCipherBossMinion>();
		}
		...
		public override void SetStaticDefaults()
		{
			...
			{
				CustomTexturePath = TerrariaHbM.AssetPath + "Textures/Bestiary/BillCipherBoss_Preview",
		...
		public override void SetDefaults()
		{
			...
			if (Main.expertMode)
			{
				NPC.lifeMax *= 2; // Double life in expert mode
			}

			if (Main.masterMode)
			{
				NPC.lifeMax *= 3; // Triple life in master mode
			}

			if (Main.getGoodWorld)
			{
				NPC.lifeMax = (int)(NPC.lifeMax * 1.5f); // Increase by 50% if using the "For The Worthy" seed
			}
			...
			// Custom boss bar
			NPC.BossBar = ModContent.GetInstance<BillCipherBossBossBar>();
		...
		public override void OnKill()
		{
			...
			NPC.SetEventFlagCleared(ref DownedBossSystem.downedBillCipherBoss, -1);
		...
		public override void HitEffect(NPC.HitInfo hit)
		{
			...
			if (NPC.life <= 0)
			{
				...
				int backGoreType = Mod.Find<ModGore>($"{Name}_Back").Type;
				int frontGoreType = Mod.Find<ModGore>($"{Name}_Front").Type;
		...
		private void SpawnMinions()
		{
			...
			{
				NPC minionNPC = NPC.NewNPCDirect(entitySource, (int)NPC.Center.X, (int)NPC.Center.Y, ModContent.NPCType<BillCipherBossMinion>(), NPC.whoAmI);
				...
				BillCipherBossMinion minion = (BillCipherBossMinion)minionNPC.ModNPC;
		...
		private void CheckSecondStage()
		{
			...
			for (int i = 0; i < Main.maxNPCs; i++)
			{
				...
				if (otherNPC.active && otherNPC.type == MinionType() && otherNPC.ModNPC is BillCipherBossMinion minion)
				{
		...
		private void ShootLaserBeam()
		{
			if (NPC.HasValidTarget && Main.netMode != NetmodeID.MultiplayerClient)
			{
				...
				int type = ModContent.ProjectileType<DeadlyLaserHostile>();
				int damage = NPC.damage / 2;
				var entitySource = NPC.GetSource_FromAI();

				// We want to offset the projectile from the center of the NPC, so we use the rotation of the NPC to offset it
				Vector2 position = NPC.Center + new Vector2(0f, 1f).RotatedBy(NPC.rotation) * NPC.height / 2f;

				Vector2 velocity = new Vector2(0f, 1f).RotatedBy(NPC.rotation);

				Projectile.NewProjectile(entitySource, position, velocity, type, damage, 0f, Main.myPlayer);
			}
		}
		...
		private void DoSecondStage_SpawnEyes(Player player)
		{
			...
			// float timerMax = Utils.Clamp((float)NPC.life / NPC.lifeMax, 0.33f, 1f) * 60;
			float timerMax = Utils.Clamp((float)NPC.life / NPC.lifeMax, 0.33f, 1f) * 120;
