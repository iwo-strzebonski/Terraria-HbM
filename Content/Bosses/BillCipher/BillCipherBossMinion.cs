...
namespace TerrariaHbM.Content.Bosses.BillCipher
{
	...
	public class BillCipherBossMinion : ModNPC
	{
		...
		public const float LaserFireTimerMax = 60;
		public ref float LaserFireTimer => ref NPC.ai[3];


		// Helper method to determine the body type
		public static int BodyType()
		{
			return ModContent.NPCType<BillCipherBoss>();
		}
		...
		private void ShootLasers()
		{
			...
			if (LaserFireTimer == 0 && Main.netMode != NetmodeID.MultiplayerClient)
			{
				Player player = Main.npc[ParentIndex].target == -1 ? null : Main.player[Main.npc[ParentIndex].target];
				...
				// We want to shoot the laser in the direction of the player, but we don't want it to be perfectly straight
				// So we add some random rotation to it
				float rotation = toPlayer.ToRotation() + Main.rand.NextFloat(-0.1f, 0.1f);

				// We also want to add some random velocity to the projectile, so it doesn't look too static
				float speed = 8f;
				Vector2 vel = rotation.ToRotationVector2() * speed;

				// Spawn the projectile
				Projectile.NewProjectile(entitySource, position, vel, type, damage, 0f, Main.myPlayer);
			}
		}
